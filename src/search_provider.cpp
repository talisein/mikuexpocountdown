        ...
        if (std::end(Miku::get_events()) != it) {
            ...
            if (diff_secs.count() <= 0) { // Event started
                ...
                if (!(*it)->is_expired()) { // Event ongoing
                    display_date = Glib::ustring::compose("until %1", std::format(date_format, std::chrono::zoned_time(current_zone(), end_time)));
                } else { // Event finished
                    display_date = Glib::ustring::compose("ended %1", std::format(date_format, std::chrono::zoned_time(current_zone(), end_time)));
                }
            } else { // Event starts in the future
                display_date = std::format(date_format, std::chrono::zoned_time(current_zone(), (*it)->start_time));
            }
